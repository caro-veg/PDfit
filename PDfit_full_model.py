...
# extract observation
df_ag_strain = df[(df['Strain']=='100042BU') & (df['Agent']=='Gepotidacin') & (df['Conc']==0.03)]
...
s = np.random.uniform(low=-10., high=10.)
q = np.random.uniform(low=-10., high=10.)
params = np.array([a, s, q])
...
rmax = np.array([0.21048, 0.21048, 0.21048, 0.21048, 0.21048])


def exp_growth(params, C, B0, rmax, times):
    a = params[0]
    s = params[0]
    q = params[2]
    r = a - np.exp(s * (C + q))
    return np.transpose(np.array([B0,])) * np.exp(np.transpose(np.array([r,])) * times)
    

# define log likelihood
def log_like(params, times, C, B0, rmax, d):
    B = exp_growth(params, C, B0, rmax, times)
    B = np.log10(B)
...
soln = minimize(nll, initial, args=(times, C, B0, rmax, logobs))


# define prior probability
def log_prior(params):
    a, s, q = params
    if (-10. < a < 10.) and (-10. < s < 10.) and (-10. < q < 10):
        return 0
    return -np.inf

# define log probability using prior probability and log likelihood
def log_prob(params, times, C, B0, rmax, d):
    lp = log_prior(params)
    if not np.isfinite(lp):
        return -np.inf
    return lp + log_like(params, times, C, B0, rmax, d)
...
# define sampler
sampler = em.EnsembleSampler(nwalkers, ndim, log_prob, args=(times, C, B0, rmax, logobs))
...
q_median = np.median(flat_samples[:,2])
q_95pc = np.percentile(flat_samples[:,2], [5, 95])
