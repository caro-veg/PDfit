...
df = pd.read_excel(r'C:\Users\cvegvari\Documents\GSK_info\Microbiology\TK-C-Preliminary-Linelist.xlsx', sheet_name='All Isolates',
                   usecols='B,C,D,E,G,H,I,J,K', skiprows=[0])
...
K = np.random.uniform(low=0., high=1e9)
params = np.array([r, K])

# define model
def logistic_growth(params, times):
    P0 = obs_array[:,0]
    K = params[1]
    r = params[0]
    return K / (1 + (K-P0)/P0 * np.exp(-r*times))
